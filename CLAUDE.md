...
- Salary: anchor conservative, state the floor of the realistic range, not the ceiling (changed 2026-07-22; reasoning in `context/salary.md`). Never give a range when a single number is asked for. Never share previous salary if asked. Set the position before writing, record it in `context/salary.md`.

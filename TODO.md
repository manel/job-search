...
- [ ] **1. Portfolio: export the remaining 3 Figma images for the Remuner DS case.** Hero + home thumbnail done and pushed to dev 2026-07-06 (62056cd). Remaining: audit board first (one pattern study: every autocomplete/input state across the product with defined variants beside them — it's the proof behind the CTO quote), token architecture (variables panel, primitive + semantic collections), pipeline evidence (passing CI run or HTML preview sandbox). Specs are in TODO comments inside `portfolio/public/cases/design-system-remuner.html`. Crop: another company's product internals.
- [ ] **3. Portfolio: finish dev, merge to main, deploy, update the pointer here.** Verified 2026-07-07: dev is clean and committed (item 2's Abacum fix went into 62056cd); the 3 missing Remuner image refs are inside HTML comments, so nothing renders broken — the case is coherent on dev, just text-heavy. Decision 2026-07-07: hold the merge until the 3 images from item 1 are exported (merge-now was offered and declined). Sequence: item 1, commit, merge dev into main, push (deploys to manel-leiva.com), update the submodule pointer here. Until merged, the Remuner DS case is invisible to recruiters: no new DS-titled application goes out, and the BVNK outreach (whose draft claims the Remuner token pipeline and links the site) is gated on this too.
- [ ] **4. Send BVNK LinkedIn outreach** (due 2026-07-03, stale 2026-07-10). Gated on item 3 (2026-07-07): the draft claims the Remuner token pipeline and links the site, but the live site has no Remuner DS case until the merge. Deploy first, then send same day — stale date leaves no slack. Find hiring manager or senior DS designer via company People tab (method in `context/outreach.md`). Draft:

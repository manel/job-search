...
**Component naming discipline** — recurring, contested. "How do we name this component? Dropdown menu? Let's not mix it with the list and toolbar elements." Shows the conceptual work required, not just visual. **Resolved (confirmed 2026-07-23):** settled by a documented naming convention/rule, not case-by-case argument. Once written down, it became the reference for future disputes instead of re-litigating each one.

**Read-only state** — debated across multiple weekly syncs. Is it a new state, a disabled state, or pre-fill automatic state? Engineers and designers had different mental models. **Resolved (confirmed 2026-07-23):** the team talked it through and reused/adapted an existing MUI/Vuetify state rather than building a new one from scratch. This reflects a general operating principle Manel applied across both systems: default to what the framework already provides, and only introduce a new state or component when the existing one is genuinely wrong for the design intent. (Same logic behind the separate MUI decision above — extend, don't rebuild, unless there's a real product reason to.)

**Deprecated components in Figma** — a real bug. Deprecated components kept appearing in search results for designers. Manel diagnosed it and proposed 3 solutions with tradeoffs, including the complexity of the "change name + remove instances + re-publish + update files + unpublish" cycle. **Resolved (confirmed 2026-07-23):** shipped fix was renaming the old component with a "⛔ - DEPRECATED" suffix and publishing the replacement under the clean name, so the deprecated one stayed visibly wrong-to-use in search during the transition. After a period, the deprecated component was unpublished. Lighter than a full rename/remove-instances/republish cycle, but still visible and safe.

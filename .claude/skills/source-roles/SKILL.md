---
name: source-roles
description: "Weekly sourcing session — batch-assess new roles and clean the pipeline. Argument: paste role URLs or JDs, one or more."
allowed-tools:
  - Read
  - Write
  - WebFetch
...
This is the weekly top-of-funnel ritual. Cadence and board list live in `context/job-boards.md`. The boards themselves can't be scanned automatically (documented limitation), so Manel browses and pastes; this skill does the assessment and bookkeeping.
...
Run each role through the `/assess-fit` framework (read that skill for the full checklist, including the DS-maturity check for DS-titled roles).
...
**Step 4 — Close with a one-paragraph funnel summary:** active count, overdue actions, what was added this week, and whether the pipeline is thin (under approximately 5 active applications means sourcing needs more volume next week).
